...
function getAcceptErrorMessage(err: unknown): { status: string; msg: string } | null {
  if (typeof err === 'object' && err !== null) {
    ...
    if (
      errorObj.message?.includes('not invited') ||
      errorObj.message?.includes('You are not invited to this OU')
    ) {
      return {
        status: 'wrong_email',
...
function validateAcceptPreconditions(ctx: AcceptPreconditionsContext): boolean {
  ...
  if (invitedEmail && user?.email?.toLowerCase() !== invitedEmail.toLowerCase()) {
    ...
    setErrorMsg(
      `You are currently logged in as ${user?.email}, but this invitation was sent to ${invitedEmail}.`,
    )
    return false
  }
  return true
...
function handleAcceptApiResult(
  ...
  if (res?.success === true) {
    if (!res.invitedEmail || user?.email?.toLowerCase() === res.invitedEmail.toLowerCase()) {
      ...
      setErrorMsg(
        `You are currently logged in as ${user?.email}, but this invitation was sent to ${res.invitedEmail}.`,
      )
    }
    return true
  }
    ...
    const checkInvitationStatus = async () => {
      ...
      try {
        ...
        if (
          ...
          setStatus('wrong_email')
          setErrorMsg(
            `You are currently logged in as ${user.email}, but this invitation was sent to ${data.recipientEmail}.`,
          )
          return
        }
      ...
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-xl rounded-2xl p-8 flex flex-col items-center space-y-6 transition-colors">
        ...
        {status === 'loading' && (
          <div className="flex items-center justify-center py-2 text-zinc-500 dark:text-zinc-300">
            <svg
              className="animate-spin h-5 w-5 text-orange-600 mr-2"
          ...
          <>
            <p className="text-sm text-zinc-600 dark:text-zinc-300 text-center">
              ...
              {!isAuthenticated && (
                <>
                  <br />
                  Please sign in or register with the invited email address first.
                </>
              )}
              {isAuthenticated && (
                <>
                  <br />
                  Click <b>Accept</b> to join.
                </>
              )}
            </p>
        ...
        {status === 'wrong_email' && (
          <div className="flex flex-col items-center space-y-4">
            <div className="text-red-600 dark:text-red-400 text-center">{errorMsg}</div>

            <div className="text-sm text-zinc-600 dark:text-zinc-300 text-center">
              Please sign in with the email address that was invited or ask for a new invitation to
              your current email.
            </div>
