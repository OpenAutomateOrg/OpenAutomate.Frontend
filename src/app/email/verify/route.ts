...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!token) {
      ...
      return NextResponse.redirect(`${new URL(config.paths.auth.emailVerified, request.nextUrl.origin)}?success=false&reason=missing-token`)
    }
    ...
    if (redirectUrl) {
      ...
      return NextResponse.redirect(`${new URL(config.paths.auth.emailVerified, request.nextUrl.origin)}?success=true`)
    }
  } catch (error) {
    console.error('Error during email verification:', error)
    // In case of error, redirect to the error page
    return NextResponse.redirect(`${new URL(config.paths.auth.emailVerified, request.nextUrl.origin)}?success=false&reason=server-error`)
  }
} 