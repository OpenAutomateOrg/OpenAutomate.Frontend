 ...
 */
function isODataResponse(obj: unknown): obj is { value: AssetResponseDto[]; '@odata.count'?: number } {
    return (
        typeof obj === 'object' &&
        obj !== null &&
        'value' in obj &&
 ...
 */
export const getAssetsWithOData = async (options?: ODataQueryOptions): Promise<ODataResponse<AssetResponseDto>> => {
    const tenant = getCurrentTenant()
 ...
 */
export const getAssetByIdWithOData = async (id: string, options?: ODataQueryOptions): Promise<AssetResponseDto> => {
    const tenant = getCurrentTenant()
