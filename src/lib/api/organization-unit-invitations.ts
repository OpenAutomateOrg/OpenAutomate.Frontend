 ...
 */
function processODataResponse(response: unknown): {
  value: OrganizationInvitationResponse[]
  '@odata.count'?: number
} {
...
export const organizationInvitationsApi = {
  ...
  inviteUser: async (tenant: string, email: string): Promise<OrganizationInvitationResponse> => {
    return api.post<OrganizationInvitationResponse>(`${tenant}/api/organization-unit-invitation`, {
      email,
    })
  },
  ...
  checkInvitation: async (
    tenant: string,
    email: string,
  ): Promise<{ invited: boolean; status?: string }> => {
