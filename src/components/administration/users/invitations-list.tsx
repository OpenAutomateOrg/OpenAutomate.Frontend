...
} from '@/lib/api/organization-unit-invitations'
...
const invitationColumns: ColumnDef<OrganizationInvitationResponse>[] = [
  ...
  },
  {
    ...
    header: ({ column }: { column: Column<OrganizationInvitationResponse, unknown> }) => (
      <DataTableColumnHeader column={column} title="Email" />
    ),
    cell: ({ row }: { row: Row<OrganizationInvitationResponse> }) => (
      <span>{row.getValue('recipientEmail')}</span>
    ),
    enableSorting: true,
  },
  {
...
export default function InvitationsList() {
  ...
  const totalPages = useMemo(() => {
    ...
    const hasMorePages = invitations.length === pageSize && count <= pageSize * (pageIndex + 1)
  ...
  const activeFilterCount = [searchEmail, searchStatus !== 'All' ? searchStatus : ''].filter(
    Boolean,
  ).length
