...
export type LogsRow = z.infer<typeof logsSchema>

export default function LogsInterface() {
  const router = useRouter()
  const initialData: LogsRow[] = [
    {
      id: '1',
  ...
  const [data] = useState<LogsRow[]>(initialData)
  ...
  const handleRowClick = (row: LogsRow) => {
    const pathname = window.location.pathname
    const isAdmin = pathname.startsWith('/admin')
    const route = isAdmin ? `/admin/logs/${row.id}` : `/[tenant]/logs/${row.id}`
    router.push(route)
  }
