...
export default function PackageInterface() {
  ...
  const [data] = useState<PackageRow[]>(initialData)
  ...
  const handleRowClick = (row: PackageRow) => {
    const pathname = window.location.pathname
    const isAdmin = pathname.startsWith('/admin')
    const route = isAdmin ? `/admin/package/${row.id}` : `/[tenant]/package/${row.id}`
    router.push(route)
  }
